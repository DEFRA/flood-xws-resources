...
for (( i=0; i<$count; i++ )); do
   ...
   ogr2ogr -f GeoJSON -t_srs EPSG:4326 -sql "SELECT fws_tacode AS code, ta_name AS name, descrip AS description, area AS region from $layername" -fid "$i" -lco COORDINATE_PRECISION=6 "$3/$filename.json" "$1"
done
