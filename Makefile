...
syncAlerts:
	aws s3 sync ./alerts s3://xws-$(ENV)-files/alerts --profile $(AWS_PROFILE) --acl public-read

syncAreas:
	aws s3 sync ./areas s3://xws-$(ENV)-files/areas --profile $(AWS_PROFILE) --acl public-read --delete
